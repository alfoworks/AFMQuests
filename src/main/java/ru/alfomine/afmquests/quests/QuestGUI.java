...
public class QuestGUI {
    private boolean pageAccess(boolean next, PlayerContribution data) {
        if (next && AFMQuests.questDataManager.getQuestDifficulties().getLevelById(data.getLevelId()).getQuests().length / 25 > data.page) {
            return true;
        } else return !next && data.page != 0;
    }

    private int getPageSize(PlayerContribution data) {
        int overall = AFMQuests.questDataManager.getQuestDifficulties().getLevelById(data.getLevelId()).getQuests().length;
    ...
    private Inventory bakeGui(PlayerContribution data, int id) {
        ...
        QuestLevel questLvl = AFMQuests.questDataManager.getQuestDifficulties().getLevelById(data.getLevelId());
        ...
        AFMQuests.logger.debug("----QUEST BUILD----");
        for (Inventory slot: slots) {
            if (id == -1) {
                if (pageAccess(false, data) && slotN == 0) {
                    slot.set(PRE);
                    AFMQuests.logger.debug("Adding PRE");
                } else if (slotN == 0) {
                    slot.set(LVL);
                    AFMQuests.logger.debug("Adding LVL");
                }

                if (pageAccess(true, data) &&  slotN == 9 * 3 - 1) {
                    slot.set(NEX);
                    AFMQuests.logger.debug("Adding NEX");
                } else if (slotN == 9 * 3 - 1) {
                    slot.set(LVL);
                    AFMQuests.logger.debug("Adding LVL");
                }
                ...
                if (!(slotN == 0 || slotN == 9 * 3 - 1)){
                    if (slotN - 1 < questSize) {
                        ...
                        AFMQuests.logger.debug("Adding QeS");
                    }
                }
                ...
                if (slotN == 8 || slotN == 0 || slotN == 18 || slotN == 26) {
                    ...
                    slot.set(YES);
                    AFMQuests.logger.debug("Adding CON");
                } else if (slotN == 13) {
                    slot.set(LOR);
                    AFMQuests.logger.debug("Adding LOR");
                } else if (slotN == 15) {
                    NOO = ItemStack.builder()
                    ...
                    slot.set(NOO);
                    AFMQuests.logger.debug("Adding NOT");
                }
                ...
                if (slotN == 8 || slotN == 0 || slotN == 18 || slotN == 26) {
                    ...
                    slot.set(YES);
                    AFMQuests.logger.debug("Adding YES");
                } else if (slotN == 13) {
                    slot.set(LOR);
                    AFMQuests.logger.debug("Adding LOR");
                } else if (slotN == 15) {
                    slot.set(NOO);
                    AFMQuests.logger.debug("Adding NOO");
                }
            } else {
                if (slotN == 8 || slotN == 0 || slotN == 18 || slotN == 26) {
                    slot.set(LVL);
                    ...
                    slot.set(INP);
                    AFMQuests.logger.debug("Adding INP");
                } else if (slotN == 15) {
                    NOO.offer(Keys.DISPLAY_NAME, Text.of(TextColors.RED, "Abort"));
                    slot.set(NOO);
                    AFMQuests.logger.debug("Adding ABO");
        ...
        AFMQuests.logger.debug("----QUEST FINAL----");
